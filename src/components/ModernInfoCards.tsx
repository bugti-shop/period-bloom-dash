    ...
    <div className="grid grid-cols-2 gap-3">
      {cards.map((card) => {
        ...
        return (
          <div
            key={card.id}
            className={`${card.style.bgColor} ${card.style.borderColor} border rounded-xl p-3 shadow-sm`}
          >
            <div className="flex items-start justify-between mb-2">
              <div className={`w-10 h-10 ${card.style.iconBg} rounded-xl flex items-center justify-center`}>
                <Icon className="w-5 h-5 text-white" />
              </div>
              <span className={`text-xs font-medium ${card.style.textColor} bg-white/60 px-2 py-0.5 rounded-full`}>
                {card.subtitle}
              </span>
            </div>
            ...
            <p className={`text-base font-bold ${card.style.textColor}`}>
              {card.date}
            </p>
          </div>
        );
