...
const BABY_ALBUM_KEY = "baby-album-photos";
...
export const BabyAlbum = ({ onClose }: BabyAlbumProps) => {
  ...
  const loadData = () => {
    const storedPhotos = loadFromLocalStorage<BabyPhoto[]>(BABY_ALBUM_KEY) || [];
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newPhotos: BabyPhoto[] = [];

    Array.from(files).forEach((file) => {
      const reader = new FileReader();
      reader.onloadend = () => {
        const newPhoto: BabyPhoto = {
  ...
  const downloadPhoto = (photoId: string) => {
    ...
    link.download = `baby-${format(photo.timestamp, 'yyyy-MM-dd-HHmm')}.jpg`;
    link.click();
  ...
  const getPhotoStyle = (photo: BabyPhoto) => {
  ...
  const sharePhoto = async (photoId: string) => {
    try {
      ...
      const file = new File(
        [blob], 
        `baby-${format(photo!.timestamp, 'yyyy-MM-dd')}.jpg`,
        { type: 'image/jpeg' }
      );
      ...
      const shareData = {
        files: [file],
        title: "Baby Photo",
        text: shareMessage || `My baby - ${format(photo!.timestamp, 'MMMM d, yyyy')} 💗`
      };
      ...
      if (navigator.canShare && navigator.canShare(shareData)) {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `baby-${format(photo!.timestamp, 'yyyy-MM-dd')}.jpg`;
        link.click();
  ...
  const createAlbumCollage = async (): Promise<Blob | null> => {
    ...
    ctx.fillText('My Baby Album', canvas.width / 2, 60);
  ...
  const shareAlbum = async () => {
    try {
      ...
      const file = new File([blob], "baby-album.jpg", { type: 'image/jpeg' });

      const shareData = {
        files: [file],
        title: "My Baby Album",
        text: shareMessage || "My baby's precious moments 💗"
      };
  ...
  const exportToPDF = async () => {
    ...
    pdf.setFontSize(24);
    pdf.text("My Baby Album", pageWidth / 2, yPosition, { align: "center" });
    yPosition += 20;
    pdf.setFontSize(12);
    pdf.text("Precious Moments", pageWidth / 2, yPosition, { align: "center" });
    ...
    <div className="fixed inset-0 bg-background z-50 overflow-y-auto">
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          {photos.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-muted-foreground mb-4">Start building your baby's album</p>
            ...
            <label htmlFor="photo-upload" className="cursor-pointer">
              <div className="border-2 border-dashed border-border rounded-lg p-8 text-center hover:border-primary transition-colors">
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Photos will be automatically dated with current time
                </p>
              </div>
              <input
                id="photo-upload"
          ...
          <div className="space-y-6">
            {editingPhoto && (
              <div className="relative w-full h-64 bg-muted rounded-lg overflow-hidden">
                <img
                  src={photos.find(p => p.id === editingPhoto)?.imageData}
                  alt="Preview"
      ...
      <Dialog open={editingTags !== null} onOpenChange={() => setEditingTags(null)}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="flex gap-2">
              <Input
                placeholder="Add tag (e.g., first smile, bath time)..."
                value={tagInput}
            ...
            <div className="flex flex-wrap gap-2">
              {editingTags && photos.find(p => p.id === editingTags)?.tags?.map((tag) => (
      ...
      <Dialog open={sharingPhoto !== null} onOpenChange={() => setSharingPhoto(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Share {sharingPhoto === "album" ? "Album Collage" : "Photo"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
          ...
          <div className="relative w-full h-full flex items-center justify-center p-8">
            {filteredPhotos[currentSlideIndex] && (
              <div className="relative max-w-4xl max-h-full">
                <img
                  src={filteredPhotos[currentSlideIndex].imageData}
                  alt="Slideshow"
                  className="max-w-full max-h-[80vh] object-contain"
                  style={getPhotoStyle(filteredPhotos[currentSlideIndex])}
                />
                <div className="absolute bottom-0 left-0 right-0 bg-black/70 text-white p-4">
                  <p className="text-xl font-semibold">
                    {format(filteredPhotos[currentSlideIndex].timestamp, 'MMMM d, yyyy - h:mm a')}
                  </p>
                  {filteredPhotos[currentSlideIndex].caption && (
                    <p className="text-sm mt-2">{filteredPhotos[currentSlideIndex].caption}</p>
                  )}
                </div>
              </div>
            )}
